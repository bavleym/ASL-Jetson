...
try:
    ...
    while True:
        ...
        results = model.predict(source=color_image, conf=0.5, device=device, imgsz=640, verbose=False)
        ...
        if results_hands.multi_hand_landmarks:
            for hand_landmarks in results_hands.multi_hand_landmarks:
                ...
                # Extract hand coordinates and calculate square bounding box
                ...
                x_min = max(0, x_min - (max_side - width) // 2)
                y_min = max(0, y_min - (max_side - height) // 2)
                x_max = min(color_image.shape[1], x_min + max_side)
                y_max = min(color_image.shape[0], y_min + max_side)

                # Use depth to filter out irrelevant gestures
                center_x = (x_min + x_max) // 2
                center_y = (y_min + y_max) // 2
                depth_value = depth_frame.get_distance(center_x, center_y)

                if not (min_depth <= depth_value <= max_depth):
                    continue  # Skip this hand if it's outside the valid depth range
